...
- Production D1 database `frc-attendance` exists in Cloudflare with database ID `c02c0ca8-033b-435f-ae21-2d8f3b203b22`, and migrations through `0015_student_attendance_required_from.sql` have been applied remotely.
...
- Member attendance requirement start dates are deployed in Worker version `144479f8-3b4e-4760-a5d4-2f32e377e4e5` and dashboard deployment `https://a824f583.frc-attendance-dashboard.pages.dev`: migration `0015_student_attendance_required_from.sql` adds `students.attendance_required_from_date`, backfills existing members from durable roster sync dates, and new roster syncs set the date from the project-local sync date. Required scheduled meetings before that date are treated as not required/excused for that member across reports, absence drilldowns, roster percentages, notifications, exports, and dashboard labels without altering scan source data or attendance history.
