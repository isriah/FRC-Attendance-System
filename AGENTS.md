...
- Production D1 database `frc-attendance` exists in Cloudflare with database ID `c02c0ca8-033b-435f-ae21-2d8f3b203b22`, and migrations through `0016_correct_attendance_required_from_timezone.sql` have been applied remotely.
...
- Roster CSV sync currently expects `memberId,firstName,lastName`, with optional `email` and `discordUserId` roster metadata fields.
...
- The API has member email notification support: `POST /admin/notifications/meeting-absence` previews or sends absence emails for completed required scheduled meetings, and `POST /admin/notifications/member-attendance-report` previews or sends one member's current attendance report. Both track delivery audit rows in D1, skip duplicate successful sends by default, and send through Resend when configured. Production has `RESEND_API_KEY`, `EMAIL_FROM_ADDRESS=attendance@robolancers.com`, and `EMAIL_FROM_NAME=FRC Attendance` configured as Worker secrets.
- Initial Discord notification support is deployed: roster rows can store `discord_user_id`, and `POST /admin/notifications/discord/missing-members` previews or sends a single webhook message mentioning absent members for one completed required meeting. Production D1 migration `0007_student_discord_user_id.sql` has been applied. Production has `DISCORD_MISSING_MEMBERS_WEBHOOK_URL` configured as a Worker secret; use dashboard preview first before sending a real Discord ping.
- Contestable Discord bot notifications are deployed in Worker version `65155f00-751d-4e56-91c9-c77a99fb0daf`: authenticated admins can manually send one missing-member message after the meeting end plus a configurable delay (default 30 minutes), linked absent members can privately contest with the message button, and the dashboard Contests tab lets admins acknowledge, resolve, or reject contests without mutating attendance. Production D1 migration `0009_attendance_contest_idempotency.sql` is applied, and `DISCORD_BOT_TOKEN`, `DISCORD_PUBLIC_KEY`, and `DISCORD_ATTENDANCE_CHANNEL_ID` are configured.
- Audited per-member meeting attendance removal is deployed in Worker version `06dd4431-fa1a-4914-890a-94ac33805892`. Admins must provide a reason and confirm before marking a present member absent; source scan/manual events remain unchanged, and `attendance_exclusions` keeps the correction effective across derived-session rebuilds, reports, exports, and notification previews.
- Audited Discord contest approval is deployed in Worker version `0e13fb6a-e62c-4abf-b8be-71734043ef4d`: authenticated admins can approve a pending contest to create a `confirm_present` manual event, supersede any active absence exclusion without deleting its audit row, rebuild attendance normally, and resolve the contest. Reject and mark-reviewed actions do not change attendance.
- Automatic contestable Discord missing-member delivery is deployed in Worker version `e179c5ff-61fd-4bf0-9db9-f0de9b0aa9ae`: a Worker Cron Trigger runs every 10 minutes and sends one bot-delivered missing-member ping for each completed required scheduled meeting after its valid end time plus `DISCORD_MISSING_MEMBER_DELAY_MINUTES` has elapsed. The scheduled path uses `scheduled_notification_locks` for meeting-level idempotency before calling Discord and keeps existing per-member `notification_deliveries` audit rows for delivered mentions. Meetings without valid end times are skipped by automation; the authenticated admin preview/manual-send route remains available.
- Discord kiosk status updates are deployed in Worker version `0d907192-bff0-465d-af14-075f0b642609`: the same 10-minute Worker Cron Trigger maintains one persistent bot-authored message in `DISCORD_ATTENDANCE_CHANNEL_ID`. Migration `0013_discord_kiosk_status_messages.sql` stores the Discord channel/message identity, rendered hash, retry state, and per-kiosk status-change timestamps. The message is created once per configured channel and edited only when rendered status changes. The default offline threshold is `KIOSK_DISCORD_OFFLINE_THRESHOLD_MINUTES=1`, matching the dashboard's 60-second offline rule while allowing roughly four missed 15-second kiosk health reports before Discord shows offline.
- Dashboard-driven Discord Scheduled Event sync is deployed in Worker version `d9a9c5dd-e0d1-443f-b81c-7b7d7b035819` and dashboard deployment `https://5a30d83c.frc-attendance-dashboard.pages.dev`: admins can push one selected scheduled meeting or multiple selected meetings to the Discord server calendar as external Scheduled Events. Migration `0014_discord_scheduled_event_mappings.sql` stores local meeting-to-Discord event mappings so later syncs update existing events instead of creating duplicates. Events default to location `Central High School`, use meeting title/start/end/notes, and sync only creates or updates Discord events; it never deletes Discord events automatically. The Worker uses `DISCORD_GUILD_ID` when configured, otherwise it derives the guild from the bot-accessible `DISCORD_ATTENDANCE_CHANNEL_ID`.
- Member attendance requirement start dates are deployed in Worker version `efb7312a-6363-4e30-bf85-bee5aad2d127` and dashboard deployment `https://d07c23b5.frc-attendance-dashboard.pages.dev`: migrations `0015_student_attendance_required_from.sql` and `0016_correct_attendance_required_from_timezone.sql` add and time-zone-correct `students.attendance_required_from_date`, backfill existing members from durable roster sync dates, and new roster syncs set the date from the project-local sync date. Required scheduled meetings before that date are treated as not required/excused for that member across reports, absence drilldowns, roster percentages, notifications, exports, and dashboard labels without altering scan source data or attendance history.
- The API also exposes authenticated admin `POST /admin/notifications/discord/test` for a harmless Discord webhook delivery test that does not require or mutate meeting, attendance, member, or notification audit data. The payload disables all Discord allowed mentions and includes only non-secret debug metadata. Worker version `abfe2271-ebc5-4014-a007-3d07cf6eb994` includes this route/action; dashboard deployment `https://314abc98.frc-attendance-dashboard.pages.dev` includes the Discord test action plus dashboard table/report timestamp display polish.
- Dashboard is deployed to Cloudflare Pages project `frc-attendance-dashboard` at `https://frc-attendance-dashboard.pages.dev`; deployment `https://734664b5.frc-attendance-dashboard.pages.dev` adds contest approval actions to the global Contests tab and selected meeting details.
...
- Backburnered: email notifications for upcoming meetings, meeting changes, or attendance digests. Missed-meeting and member attendance report email support targets Resend for delivery; member emails still do not grant dashboard admin access.
- Backburnered: Discord scheduled-event sync, Discord interaction commands for attendance corrections, and kiosk status alerts. The first Discord slice is limited to member Discord IDs and missing-member pings for completed required meetings.
