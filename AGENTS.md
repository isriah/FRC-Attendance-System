...
- The API also exposes authenticated admin `POST /admin/notifications/discord/test` for a harmless Discord webhook delivery test that does not require or mutate meeting, attendance, member, or notification audit data. The payload disables all Discord allowed mentions and includes only non-secret debug metadata. Worker version `abfe2271-ebc5-4014-a007-3d07cf6eb994` and dashboard deployment `https://206a28c9.frc-attendance-dashboard.pages.dev` include this route/action; unauthenticated production smoke returned `401 Missing admin identity`, and no real Discord debug message was sent because no production admin Google token/session was available in terminal smoke.
