...
- Initial Discord notification support is implemented in source but requires migration/config before production use: roster rows can store `discord_user_id`, and `POST /admin/notifications/discord/missing-members` previews or sends a single webhook message mentioning absent members for one completed required meeting. Configure `DISCORD_MISSING_MEMBERS_WEBHOOK_URL` as a Worker secret before sending; without it, the endpoint remains preview-only.
